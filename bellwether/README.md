...
- `cli.py` — argument parsing; resolves a local path, git URL, or `owner/repo`; clones URLs to a temp dir (treeless) and cleans up.
...
- `detectors.py` — the static detectors (entry point, agent files, module README coverage, broken links, doc freshness) + false-positive guards.
